...
export default {
  ...
  methods: {
    loadPreferences() {
      fetch('http://127.0.0.1:8000/preferences/get_preferences/')
    ...
    updatePreferences() {
      fetch('http://127.0.0.1:8000/preferences/update_preference/', {
        method: 'POST',
        headers: {
    ...
    addPreference() {
      fetch('http://127.0.0.1:8000/preferences/add_preference/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    deletePreference() {
      fetch('http://127.0.0.1:8000/preferences/delete_preference/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ user_id: this.preferences.user_id })  // 傳遞 user_id 進行刪除
      })
      .then(response => {
        if (response.ok) {
          this.showSuccessMessage();
    ...
    queryPreferences() {
      let query = `http://127.0.0.1:8000/preferences/query_preferences/?`;
...
<style scoped src="../assets/css/PersonalPreference.css"></style>
